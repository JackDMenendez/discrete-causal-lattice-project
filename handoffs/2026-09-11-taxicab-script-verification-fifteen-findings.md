...
flags:
  ...
  - "PUBLICITY: dcl-mathematics is a PUBLIC repo, so 5141d4a made the full cubic taxicab architecture document publicly visible. That followed the author's explicit instruction and the DCL program publishes openly, but a standing embargo note distinguishes publishable epistemology from not-yet-publishable architecture. Raised for the author's awareness; reversible only by rewriting public history, so treat as done unless the author says otherwise. The new dcl-cubic-taxicab repo was created PRIVATE by default as the reversible direction."
decisions:
