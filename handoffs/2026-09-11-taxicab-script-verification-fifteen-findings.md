...
decisions:
  ...
  - "Author (2026-09-11): the eleven scripts get a NEW PAPER REPO rather than going into dcl-mathematics. Created as JackDMenendez/dcl-cubic-taxicab (public, main). Rationale: dcl-mathematics' CLAUDE.md scopes the no-code rule to Python numerics and the repo is deliberately paper + Lean (no tracked .py at all), while the program-wide discipline says a derivation ships WITH its verification. A new paper repo satisfies both."
