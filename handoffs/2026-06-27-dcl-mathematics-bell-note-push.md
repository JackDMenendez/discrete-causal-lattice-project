...
flags:
  - "SERIALIZATION (updated 2026-06-28): dcl-mathematics local main is ahead 1 (commit 3ed082a, the Bell note, committed-unpushed) and has uncommitted NOTE files for the PM to commit. As of this writing: (a) notes/dcl_mathematics_scope_and_outline.md (roster edit), (b) notes/type_theoretic_foundations_directed_cubical.md, (c) notes/manifesto_two_transcendentals.md, (d) notes/phase_clock_and_bipartite_parity.md. ROBUST RULE: commit ALL new/modified files under notes/ (anything there is intended), and do NOT commit the untracked src/ — that is JACK'S ACTIVE AGDA WORK. So: `git add notes/` (verify nothing else sneaks in), then push."
...
- Uncommitted (PM to commit — ALL of `notes/`): `notes/dcl_mathematics_scope_and_outline.md`
  (roster cross-refs), `notes/type_theoretic_foundations_directed_cubical.md`,
  `notes/manifesto_two_transcendentals.md`, and
  `notes/phase_clock_and_bipartite_parity.md` (the Agda phase-clock POC roadmap).
...
## → Consumer actions
- [ ] Repo `dcl-mathematics`: `git status` / `git diff` first. ROBUST RULE —
      commit ALL new/modified files under `notes/` (currently: scope-outline
      roster, type-theory foundations, two-transcendentals manifesto, phase-clock
      POC roadmap). Untracked `src/` is **Jack's active Agda work — leave it
      untracked, do NOT commit it.**
- [ ] `git add notes/` (verify nothing else is staged — esp. not `src/`), commit
      (e.g. `notes: transcendentals manifesto, type-theory + phase-clock threads,
      roster cross-refs`), then push `dcl-mathematics` `main` to origin.
